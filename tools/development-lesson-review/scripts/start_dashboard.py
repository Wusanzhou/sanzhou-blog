...
<body>
  ...
  <script>
    ...
    function parseRecordDate(title) {
    ...
    function renderStructured() {
      ...
      const filteredRecords = statusField && activeFilter !== "全部"
        ...
        : records;
      const visibleRecords = filteredRecords.filter(r => matchSearch(r.raw) && matchTime(r.title));
      ...
      structuredView.innerHTML = `
        ...
        ${visibleRecords.length ? visibleRecords.map(recordHtml).join("") : `<div class="empty">当前筛选条件下没有记录。</div>`}
      `;
    }

    function buildSummary(records, counts, statusField) {
      const total = `<span class="pill">记录：${records.length}</span>`;
      if (!statusField) return total;
    ...
    function renderSections() {
      ...
      structuredView.innerHTML = `
        ...
        ${sections.filter(section => matchSearch(section.raw) && matchTime(section.title)).map(section => `
          <article class="record">
            <div class="record-head">
    ...
    function renderMemory() {
      ...
      structuredView.innerHTML = `
        ...
        ${blocks.filter(block => matchSearch(block.title + "\n" + block.body) && matchTime(block.title)).map(block => `
          <article class="record">
            <div class="record-head">
    ...
    async function loadFile() {
      ...
      filePath.innerHTML = `<div class="desc">${escapeHtml(meta.description)}</div><div class="file-ref">${escapeHtml(meta.path)}</div>`;
      editor.value = "";
      dirty = false;
      setStatus("正在读取...");
      try {
