...
usage() {
  cat <<'EOF'
用法：
  ./scripts/install.sh --repo /path/to/project [--repo /path/to/another] [--paused] [--upgrade] [--dry-run] [--utc-hour 10] [--model gpt-5.5] [--maintenance-dir /path/to/maintenance]
...
参数：
  ...
  --utc-hour H     写入调度器的 UTC 小时。北京时间 18:00 通常对应 10。
...
if [[ "$DRY_RUN" == "true" ]]; then
  ...
  echo "[dry-run] 调度: UTC ${UTC_HOUR}:00"
...
echo "升级模式: $UPGRADE"
echo "调度: UTC ${UTC_HOUR}:00；若调度器按 UTC 解释，UTC 10:00 对应北京时间 18:00。"
echo "仓库: ${REPOS[*]}"
